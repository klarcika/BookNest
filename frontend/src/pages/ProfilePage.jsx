...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const shelvesRes = await bookshelfApi.get(`user/${encodeURIComponent(userRes?.data?._id)}?t=${Date.now()}`,
          { headers: { Authorization: `Bearer ${localStorage.getItem('jwtToken')}` } }
        );

        const s = shelvesRes?.data?.shelves || { wantToRead: [], reading: [], read: [] };
        setBookshelves({
          wantToRead: s.wantToRead || [],
          currentlyReading: s.reading || [], 
          read: s.read || [],
        });
      } catch (err) {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const ids = [
          ...(bookshelves.wantToRead || []).map(i => i.bookId),
          ...(bookshelves.currentlyReading || []).map(i => i.bookId),
          ...(bookshelves.read || []).map(i => i.bookId),
        ].filter(Boolean);
        ...
        const results = await Promise.all(uniqueIds.map(async (bid) => {
          try {
            const r = await bookApi.get(`/${bid}`, {
              headers: { Authorization: `Bearer ${localStorage.getItem('jwtToken')}` }
            });
            return r.data;
          } catch {
  ...
  const renderBooks = (bookItems = [], currentShelf) => (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {bookItems.map((item, idx) => {
        const book = getBook(item?.bookId);
        if (book) {
          return (
            <div key={`${item.bookId}-${idx}`}>
              <BookCardDetails
                book={book}
        ...
        const when = currentShelf === 'wantToRead' ? item?.date : item?.dateAdded;
        return (
          <div key={`${item.bookId}-${when || idx}`} className="border rounded p-3 bg-white shadow-sm">
            <div className="text-xs uppercase tracking-wide text-gray-500">{shelfLabels[currentShelf] || currentShelf}</div>
            <div className="mt-1 font-semibold break-all">{item.bookId}</div>
            {when && (
            ...
            <div className="mt-3 flex gap-2">
              {['wantToRead','currentlyReading','read'].filter(s => s !== currentShelf).map(target => (
                <button
                  key={target}
                  className="text-xs px-2 py-1 rounded border hover:bg-gray-50"
                  onClick={() => handleMove(item.bookId, currentShelf, target)}
                >
                  Move to {shelfLabels[target]}
                </button>
              ))}
            </div>
          </div>
